...
func TestLast(t *testing.T) {
	...
	ss := NewStatStore(10, time.Minute, 60, []float64{})
	assert := assert.New(t)
	...
	last, err := ss.Last()
	assert.Error(err)
	assert.Equal(last, store.TimePoint{})

	// Put 5 Points in the same minute. Average: 10029, Max: 50000
	assert.NoError(ss.Put(store.TimePoint{
		Timestamp: now,
		Value:     uint64(55),
	}))
	assert.NoError(ss.Put(store.TimePoint{
	...
	assert.Error(ss.Put(store.TimePoint{
...
func TestMax(t *testing.T) {
	...
	ss := NewStatStore(50, time.Minute, 5, []float64{})
	assert := assert.New(t)
	now := time.Now().Truncate(time.Minute)
...
func TestGet(t *testing.T) {
	...
	ss := NewStatStore(100, time.Minute, 5, []float64{})
	assert := assert.New(t)
	...
	res = ss.Get(zeroTime, zeroTime)
	require.Len(res, 1)
	assert.Equal(res[0], store.TimePoint{
		Timestamp: now,
		Value:     uint64(200),
	})
	...
	assert.Equal(res[1], store.TimePoint{
		Timestamp: now,
		Value:     uint64(200),
	})
	...
	assert.Equal(res[2], store.TimePoint{
		Timestamp: now,
		Value:     uint64(200),
	})
	...
	assert.Equal(res[3], store.TimePoint{
		Timestamp: now.Add(time.Minute),
		Value:     uint64(600),
	})
	assert.Equal(res[4], store.TimePoint{
		Timestamp: now,
		Value:     uint64(200),
	})
	...
	res = ss.Get(now.Add(10*time.Minute), now)
	assert.Len(res, 0)

	// Invocation with mid-length start-end range
	res = ss.Get(now.Add(3*time.Minute), now.Add(5*time.Minute))
	assert.Len(res, 3)
	assert.Equal(res[0], store.TimePoint{
		Timestamp: now.Add(5 * time.Minute),
		Value:     uint64(600),
	...
	res = ss.Get(now.Add(2*time.Minute), now.Add(6*time.Minute))
	assert.Len(res, 4)
	assert.Equal(res[0], store.TimePoint{
		Timestamp: now.Add(6 * time.Minute),
		Value:     uint64(800),
	...
	res = ss.Get(now.Add(-2*time.Minute), now.Add(time.Minute))
	assert.Len(res, 0)
...
func TestPercentile(t *testing.T) {
	// epsilon: 50, resolution: 1 minute, total: 5 minutes, no percentiles
	ss := NewStatStore(50, time.Minute, 5, []float64{0.5, 0.95})
	assert := assert.New(t)
	now := time.Now().Truncate(time.Minute)

	// Invocation with nothing in the statStore - error
	pc, err := ss.Percentile(0.95)
	assert.Error(err)
	...
	pc, err = ss.Percentile(0.2)
	assert.Error(err)
	assert.Equal(pc, uint64(0))

	// Invocation with one element in the statStore
	pc, err = ss.Percentile(0.5)
	assert.NoError(err)
	assert.Equal(pc, 200)
	pc, err = ss.Percentile(0.95)
	assert.NoError(err)
